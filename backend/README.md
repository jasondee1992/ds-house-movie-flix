...
The video's parent folder is its primary media directory. Its displayed title is the exact video filename without the extension; a terminal parenthesized year such as `Movie Name (2026)` is also parsed for the year field. Named posters (`poster.*`, `folder.*`, `cover.*`, or a matching movie name) are preferred, but any common image in the movie folder can serve as its poster. Horizontal backdrops (`backdrop.*`, `background.*`, `fanart.*`, or `hero.*`) and `.srt`, `.vtt`, `.ass`, and `.ssa` subtitles beside the video or in `Subs/` are discovered without exposing their paths.

The recommended library layout is `MOVIES/<Year>/<Movie Name (Year)>/`. Known movies receive cataloged genres automatically. For a new or unmatched movie, add a UTF-8 `genre.txt` or `genres.txt` beside the video (for example, `Horror, Comedy`); the next automatic scan places it into those category rows. Movies without recognized or sidecar metadata safely use `Other`.
